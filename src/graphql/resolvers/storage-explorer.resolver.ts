/* eslint-disable @typescript-eslint/no-unsafe-assignment */
import { Logger } from '@map-colonies/js-logger';
import { IFile, IFileMap } from '@map-colonies/storage-explorer-middleware';
import { IConfig } from 'config';
import { container } from 'tsyringe';
import { Resolver, Query, Arg, ObjectType, Field } from 'type-graphql';
import { GraphQLJSONObject } from 'graphql-type-json';
import { Services } from '../../common/constants';
import { StorageExplorerManager } from '../../common/storage-explorer-manager/storage-explorer-manager';
import { ExplorerGetDecryptedId, ExplorerGetDirectory, ExplorerGetDirectoryById, ExplorerGetFile, ExplorerGetFileById } from '../inputTypes';

@ObjectType()
class FileMap {
  @Field((type) => String)
  public data: IFileMap<IFile>;
}

@ObjectType()
class File {
  @Field((type) => GraphQLJSONObject)
  public data: Record<string, string>;
}

@ObjectType()
class DecryptedId {
  @Field((type) => String)
  public data: string;
}

@Resolver()
export class StorageExplorerResolver {
  private readonly logger: Logger;
  private readonly config: IConfig;
  private readonly storageExplorerManager: StorageExplorerManager;
  public constructor() {
    this.logger = container.resolve(Services.LOGGER);
    this.config = container.resolve(Services.CONFIG);
    this.storageExplorerManager = container.resolve(StorageExplorerManager);
  }

  @Query((type) => FileMap)
  public async getDirectory(@Arg('data') data: ExplorerGetDirectory): Promise<FileMap> {
    const { pathSuffix, type } = data;
    const dirContent = await this.storageExplorerManager.getDirectory({ pathSuffix, type });

    return dirContent;
  }

  @Query((type) => FileMap)
  public async getDirectoryById(@Arg('data') data: ExplorerGetDirectoryById): Promise<FileMap> {
    const { id, type } = data;
    const dirContent = await this.storageExplorerManager.getDirectoryById({ id, type });

    return dirContent;
  }

  @Query((type) => File)
  public async getFile(@Arg('data') data: ExplorerGetFile): Promise<File> {
    const { pathSuffix, type } = data;
    const dirContent = await this.storageExplorerManager.getFile({ pathSuffix, type });

    return { data: dirContent };
  }
  @Query((type) => File)
  public async getFileById(@Arg('data') data: ExplorerGetFileById): Promise<File> {
    const { id, type } = data;
    const dirContent = await this.storageExplorerManager.getFileById({ id, type });

    return { data: dirContent };
  }

  @Query((type) => DecryptedId)
  public async getDecryptedId(@Arg('data') data: ExplorerGetDecryptedId): Promise<DecryptedId> {
    const { id, type } = data;
    const dirContent = await this.storageExplorerManager.getDecryptedId({ id, type });

    return dirContent;
  }
}
