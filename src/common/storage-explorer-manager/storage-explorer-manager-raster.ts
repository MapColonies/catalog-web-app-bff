  ...
  public async getDirectory(data: ExplorerGetByPath, ctx: IContext): Promise<File[]> {
      ...
      .then((files) => {
        if (data.fileType != undefined) {
          return files.map((file) => {
            const fileTypeSelectable = data.fileType != undefined ? path.extname(file.name) === `.${data.fileType}` : true;
    ...
    if (data.fileType != undefined) {
      filteredRes = res.filter((file) => {
        return SELECTION_TYPE_SATELITES[data.fileType as FileType].some((ext) => {
