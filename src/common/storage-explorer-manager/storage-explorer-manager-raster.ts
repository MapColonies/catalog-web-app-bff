...
export class StorageExplorerManagerRaster implements IStorageExplorerManagerService {
  ...
  public async getDirectory(data: ExplorerGetByPath, ctx: IContext): Promise<File[]> {
    ...
    const res = await requestExecutor(
      ...
      .then((files) => {
        if (data.selectableFile) {
          return Object.values(
            files.map((file) => {
              const selectable = !file.isDir && file.name !== 'metadata.json' && file.name.endsWith(data.selectableFile as string);
    ...
    const filteredRes = res.filter((file) => {
      if (data.selectableFile?.endsWith('shp')) {
        return SHAPEFILE_ALLOWED_EXTENSIONS.some((ext) => file.name.includes(ext) || file.isDir);
      } else {
        return file.name.endsWith(data.selectableFile as string) || file.isDir;
      }
    });
