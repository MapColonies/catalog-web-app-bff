...
import { CatalogRecordType, fieldTypesVector, Services } from '../common/constants';
import { IContext, IService } from '../common/interfaces';
import { CSWCatalog } from '../graphql/csw';
import { SearchOptions } from '../graphql/inputTypes';
import { extractErrorMessage, requestExecutor, urlHandler } from '../utils';
...
export class CswWfsClientWrapper {
  ...
  public async getRecords(ctx: IContext, opts?: SearchOptions, start?: number, end?: number, resultType?: ResultType): Promise<CSWCatalog> {
    ...
    try {
      ...
      return {
        ...
        cswQuerySummary: {
          numberOfRecordsMatched: parsedEntities.length,
          nextRecord: 0,
          numberOfRecordsReturned: parsedEntities.length,
        },
      };
    } catch (err) {
  ...
  private readonly transformRecordsToEntity = async (wfsArray: Record<string, unknown>[], ctx: IContext): Promise<VectorBestRecord[]> => {
    const {
      isId,
      isType,
      isProductName,
